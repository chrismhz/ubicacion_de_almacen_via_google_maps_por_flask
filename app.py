...
def calcular_rutas():
    ubicacion_almacen = request.form['almacen']
    latitud, longitud = map(float, ubicacion_almacen.split(',')) # Divide la ubicación en latitud y longitud

    # Ahora puedes usar latitud y longitud en tu algoritmo VRP para calcular las rutas
    # Ejecuta vrp_voraz() con la nueva ubicación del almacén

    rutas = vrp_voraz()

    # Renderiza una nueva plantilla que muestre los resultados en nodos
    return render_template('mostrar_rutas.html', rutas=rutas, latitud=latitud, longitud=longitud)
